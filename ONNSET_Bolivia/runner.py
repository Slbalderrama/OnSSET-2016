...
if choice == 1:
    ...
    for scenario in Scenarios:
        ...
        end_year_pop = ScenarioParameters.iloc[popIndex]['PopEndYear'] # Population in the last year
        ...
        Hybrid_calc = Technology(om_of_td_lines=0.02,
                                    distribution_losses=0.05,
                                    connection_cost_per_hh=125,
                                    base_to_peak_load_ratio=0.5,
                                    capacity_factor=0.7,
        ...
        for year in yearsofanalysis:
            ...
            # year of the analysis, calculation of demands and other data
            # Sergio: Reviewed
            ...
            # key line
            # calulates the LCOE of off grid solutions
            onsseter.calculate_off_grid_lcoes(mg_hydro_calc, mg_wind_calc, mg_pv_calc, sa_pv_calc, 
                                              mg_diesel_calc, sa_diesel_calc, Hybrid_calc, year, start_year, 
                                              end_year, time_step)
            # grid calculations cost
            onsseter.pre_electrification(grid_calc, grid_price, year, time_step, start_year)
            # min grid distance, electrification order # Runs the grid extension algorithm
            onsseter.run_elec(grid_calc, max_grid_extension_dist, year, start_year, 
                              end_year, time_step, grid_cap_gen_limit)

            # Decition of which thecnology to use
            onsseter.results_columns(mg_hydro_calc, mg_wind_calc, mg_pv_calc, sa_pv_calc, 
                                     mg_diesel_calc, sa_diesel_calc, grid_calc, year)
            # invesment cost 
            onsseter.calculate_investments(mg_hydro_calc, mg_wind_calc, mg_pv_calc, sa_pv_calc, 
                                           mg_diesel_calc, sa_diesel_calc, grid_calc, year, 
                                           end_year, time_step)
            # Electrification status 
            onsseter.apply_limitations(eleclimit, year, time_step, prioritization)
            # Invesment costs
            onsseter.final_decision(mg_hydro_calc, mg_wind_calc, mg_pv_calc, 
        ...
        Data = onsseter.df.copy()
        
        Data = Data.loc[Data['MinimumOverall2025']!= 'Grid_extension2025']
        Data = Data.loc[Data['Elevation']< 800]
        Data['HouseHolds'] = Data['Pop2025']/Data['NumPeoplePerHH']
        # check quantity of electrified places with minigrids in lowlands
        Data = Data.loc[Data['HouseHolds']>30]
        Data = Data.loc[Data['HouseHolds']<250]
        
        Data.to_csv('Cooking_Microgrids_Onsset.csv')