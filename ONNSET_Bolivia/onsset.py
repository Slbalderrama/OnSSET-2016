...
class SettlementProcessor:
    ...
    def set_scenario_variables(self, year, num_people_per_hh_rural, num_people_per_hh_urban, 
                               time_step, start_year, urban_elec_ratio, rural_elec_ratio, 
        ...
        """
        # Coping the population in 2025, depending on end_year_pop
        # end yeat pop is the population on the last year
        if end_year_pop == 0:
        ...
        if year - time_step == start_year:
            ...
            # for the electricate population, calculate the number of new conections
            # From the electrify quantity minus the population on the year of analisys
            self.df.loc[(self.df[SET_ELEC_FUTURE_ACTUAL + "{}".format(
                year - time_step)] == 1), SET_NEW_CONNECTIONS + "{}".format(year)] = \
                (self.df[SET_POP + "{}".format(year)] - self.df[SET_ELEC_POP])
            # Assign new connections to those that are not currently electrified, it is equal
            # to the analysis year
        ...
        if max(self.df['PerCapitaDemand']) == 0:
            ...
            # 0 is rural, 1 is rural and 2 is urban
            ...
            # choosing from a tier demand or one made for the project
        ...
        # Calculate the 'EnergyPerSettlement2025' by multiplying the household demand 
        # by the new connections
        ...
        # if year - time_step == start_year:
        # Calculate the 'EnergyPerSettlement2025' by multiplying the household demand 
        # by the conecctions on the target year
    ...
    def calculate_off_grid_lcoes(self, mg_hydro_calc, mg_wind_calc, mg_pv_calc,
                                 sa_pv_calc, mg_diesel_calc, sa_diesel_calc, Hybrid_calc, year, 
                                 start_year, end_year, timestep):
        """
        Calcuate the LCOEs for all off-grid technologies, and calculate the minimum, 
        so that the electrification
        ...
        hydro_df[hydro_used] = 0 # creating a column with 0, for the hydro used
        hydro_df = hydro_df.set_index(SET_HYDRO_FID) #chaging the index 
...
############## I have to add the new function here!
          
        logging.info('Calculate minigrid hybrid LCOE')
        self.df['Hybrid_Cost'] = self.df.apply(
            lambda row: Hybrid_calc.get_lcoe(energy_per_cell=row[SET_ENERGY_PER_CELL + "{}".format(year)],
                                                start_year=year - timestep,
                                                end_year=end_year,
                                                people=row[SET_POP + "{}".format(year)],
                                                 new_connections=row[SET_NEW_CONNECTIONS + "{}".format(year)],
                                                ...
                                                grid_cell_area=row['GridCellArea'],
                                                conf_status=row[SET_CONFLICT],
                                                travel_hours=row[SET_TRAVEL_HOURS], hybrid=True, GHI = row[SET_GHI]), axis=1)            
        self.df['Hybrid_Transmission_Cost'] = self.df.apply(
            lambda row: Hybrid_calc.get_lcoe(energy_per_cell=row[SET_ENERGY_PER_CELL + "{}".format(year)],
                                                start_year=year - timestep,
                                                end_year=end_year,
                                                people=row[SET_POP + "{}".format(year)],
                                                ...
                                                grid_cell_area=row['GridCellArea'],
                                                conf_status=row[SET_CONFLICT],
                                                travel_hours=row[SET_TRAVEL_HOURS], hybrid=True, 
                                                GHI = row[SET_GHI],
                                                get_invesment_transmission=True  ), axis=1)     
        self.df['Hybrid_NPC'] = self.df.apply(
            lambda row: Hybrid_calc.get_lcoe(energy_per_cell=row[SET_ENERGY_PER_CELL + "{}".format(year)],
                                                start_year=year - timestep,
                                                end_year=end_year,
                                                people=row[SET_POP + "{}".format(year)],
                                                ...
                                                travel_hours=row[SET_TRAVEL_HOURS], hybrid=True, 
                                                GHI = row[SET_GHI],
                                                get_investment_cost=True  ), axis=1)      
            ...
            if choice == 1:  # Prioritize grid intensification first, then lowest investment per capita in combination with dist to cities for better calibration
                ...
                if sum(self.df[self.df[SET_ELEC_FUTURE_GRID + "{}".format(year - timestep)] == 1][
                    ...
                    while abs(elecrate - eleclimit) > 0.01:
                        ...
                        elecrate += sum(self.df[(self.df[SET_INVEST_PER_CAPITA + "{}".format(year)] > min_investment) &
                                               (self.df[SET_ELEC_FUTURE_GRID + "{}".format(year - timestep)] == 0) &
                                               (self.df[SET_TRAVEL_HOURS] < min_dist_to_cities)][SET_ELEC_POP]) / \
                                   self.df[SET_POP + "{}".format(year)].sum()
                        elecrate += sum(self.df[(self.df[SET_INVEST_PER_CAPITA + "{}".format(year)] < min_investment) &
                                                (self.df[SET_ELEC_FUTURE_GRID + "{}".format(year - timestep)] == 0) &
                                                (self.df[SET_TRAVEL_HOURS] > min_dist_to_cities)][SET_ELEC_POP]) / \
                    ...
                    elecrate = self.df.loc[self.df[SET_LIMIT + "{}".format(year)] == 1, SET_POP + "{}".format(year)].sum() / \
                               self.df[SET_POP + "{}".format(year)].sum()
                    elecrate+= self.df.loc[self.df[SET_LIMIT + "{}".format(year)] == 0, SET_ELEC_POP].sum() / \
                               self.df[SET_POP + "{}".format(year)].sum()
                else:
                    print(
